...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (errors.length > 0) console.error('[form-a] Partial failure:', errors);
  ...
  const response = await fetch('https://api.notion.com/v1/pages', {
    ...
    body: JSON.stringify({
      ...
      properties: {
        ...
        Source: { rich_text: [{ text: { content: 'Form A - Quick Intake' } }] },
...
async function sendSlackAlert({ name, email, brand, tier, submitted }) {
  ...
  const text = ['🔥 *New Form A Submission*', '', `*Name:* ${name}`, `*Email:* ${email}`, `*Brand:* ${brand || 'N/A'}`, `*Tier Interest:* ${tier || 'N/A'}`, `*Submitted:* ${date}`, '', 'View in Notion -> Client Pipeline'].join('\n');
...
async function sendConfirmationEmail({ name, email }) {
  ...
  await transporter.sendMail({
    ...
    html: `<div style="font-family:sans-serif;max-width:560px;color:#111;"><p>Hi ${name},</p><p>Your interest in Flood Systems has been received. We will be in touch within 24 hours.</p><p><a href="https://floodinternational.com">floodinternational.com</a></p><p>Talk soon,<br><strong>Nicholas King</strong><br>Flood Systems</p></div>`,
  });
                                                  }
