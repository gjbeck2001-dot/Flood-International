...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Form B is a high-signal event — person spent time filling 49 questions.
    // Write to CRM and fire a priority Slack alert. No auto-email here (discovery call comes next).
...
async function writeToNotion({ name, email, phone, brand, tier, website, goal, submitted }) {
  // Build notes field from additional Form B data
  const notes = [
  ...
  const response = await fetch('https://api.notion.com/v1/pages', {
    ...
    body: JSON.stringify({
      ...
      properties: {
        ...
        Source: {
          rich_text: [{ text: { content: 'Form B — Full Intake' } }],
        },
        Notes: {
          rich_text: [{ text: { content: notes } }],
        },
        Status: {
          select: { name: 'Intake Complete' },
        },
        'Intake Complete': {
          checkbox: true,
        },
...
async function sendSlackAlert({ name, email, brand, tier, submitted }) {
  ...
  // Form B is a serious lead — different tone than Form A alert
  const text = [
